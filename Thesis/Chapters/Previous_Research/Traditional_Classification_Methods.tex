...
\begin{algorithm}
    ...
    \label{alg:pr_IDT}
    \begin{algorithmic}[1]
        \Procedure{IDT}{$vec,diThresh,duThresh$} %\Comment{The IDT of vector, with thresholds}
            \While{$vec$ \textbf{not} $empty$}
                \State $window \gets$ first window to cover $duThresh$
                \If{DISPERSION($window$) $< diThresh$}
                    \While{DISPERSION($window$) $< diThresh$}
...
\begin{algorithm}
    ...
    \label{alg:pr_IVT}
    \begin{algorithmic}[1]
        \Procedure{IVT}{$vec,vThresh$} %\Comment{The IVT of vector, with velocity threshold}
